            ...
            foreach (Match match in _parseOutput.Matches(output))
            {
                ...
                else
                {
                    ...
                    if (!(_currentOutput is null)
                        && _currentOutput.Count > 0
                        && _currentOutput[_currentOutput.Count - 1].Length == 0)
                    {
                        // Remove the empty line added by the UnitTestLauncher after output from a
                        // test/setup/cleanup method
                        _currentOutput.RemoveAt(_currentOutput.Count - 1);
                    }
                    ...
                    else if (_testClassPhase != TestClassPhases.NoMansLand)
                    {
                        ...
                        string fqn = match.Groups["fqn"].Value;
                        string fqnClass = fqn.Substring(0, fqn.LastIndexOf('.'));
                        string fqnMethod = fqn.Substring(fqnClass.Length + 1);
                        int dataRowIndex = match.Groups["type"].Value == "M"
                            ? -1
                            : int.Parse(match.Groups["row"].Value);

                        if (_currentTestCase is null
                            || _currentTestCase.Group.FullyQualifiedName != fqnClass
                            || _currentTestCase.MethodName != fqnMethod
                            || _currentTestCase.DataRowIndex != dataRowIndex)
                        {
                            (int selectionIndex, TestCase testCase) = (from tc in _testCases.TestCases
                                                                       where tc.testCase.Group.FullyQualifiedName == fqnClass
                                                                             && tc.testCase.MethodName == fqnMethod
                                                                             && tc.testCase.DataRowIndex == dataRowIndex
