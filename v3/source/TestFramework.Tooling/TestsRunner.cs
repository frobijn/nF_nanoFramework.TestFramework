    ...
    public abstract class TestsRunner
    {
            ...
            ITestsExecutionLogger logger)
        {
            try
            {
                ...
                {
                    logger.Log(LoggingLevel.Error, $"An unexpected error occurred: {ex.Message}");
                }

                if (logger.HasErrors || application is null)
                {
                    logger.Log(LoggingLevel.Error, "Test is not executed as the unit test launcher could not be created.");
                    RunAsync(() =>
