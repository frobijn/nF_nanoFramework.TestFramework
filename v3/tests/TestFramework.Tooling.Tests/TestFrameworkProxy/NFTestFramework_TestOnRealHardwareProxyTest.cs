        ...
        public void TestOnRealHardwareProxy_nF_TestFramework()
        {
            ...
            TestCase actual = (from tc in testCases.TestCases
                               where tc.ShouldRunOnRealHardware
                                     && tc.FullyQualifiedName.StartsWith("TestFramework.Tooling.Execution.Tests.TestWithNewTestMethodsAttributes.MethodToRunOnRealHardware")
            ...
            actual = (from tc in testCases.TestCases
                      where tc.ShouldRunOnRealHardware
                            && tc.FullyQualifiedName.StartsWith("TestFramework.Tooling.Execution.Tests.TestWithFrameworkExtensions.TestDeviceWithSomeFile")
                      select tc).FirstOrDefault();
            if (actual is null)
            {
                Assert.Inconclusive();
