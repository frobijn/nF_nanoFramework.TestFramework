	...
	public static class RealHardwareSerialPorts
	{
		...
		public static IEnumerable<string> GetSerialPortNames (int numberExisting, int numberNonExisting)
		{
			...
			if (numberFound < numberExisting)
			{
				if (numberExisting == 1)
				{
					Assert.Inconclusive ($"The test requires a real hardware device that does not seem to be connected: no COM# port with # < 30 is present.");
				}
				else
				{
					Assert.Inconclusive ($"The test requires more ({numberExisting}) real hardware devices than available; only {numberFound} COM# port with # < 30 present.");
				}
			}
